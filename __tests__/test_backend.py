...
def test_build_all():
    response = client.post(
        "/build-all", json={"docker_compose_path": "path/to/docker-compose.yml"}
    )
...
def test_is_container_running():
    response = client.post(
        "/is-container-running", json={"container_name": "my_container"}
    )
    docker = client.get("/is-docker-running")
...
def test_clear():
    response = client.post(
        "/clear", json={"docker_compose_path": "path/to/docker-compose.yml"}
    )
    assert response.status_code == 422
    # assert response.json() is True
